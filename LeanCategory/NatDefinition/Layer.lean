...
scoped notation X "^⋆" n => Nat.repeat NatDefinition.star n X

def boundary : TopBottom → Layer C → N C
  | .Top, ⟨L, _, Y, s, _, R⟩ => L.tensor ((Y^⋆s).tensor R)
  | .Bottom, ⟨L, X, _, s, _, R⟩ => L.tensor ((X^⋆s).tensor R)

@[simp]
lemma boundary_top (l : Layer C) :
    l.boundary .Top = l.left.tensor ((l.cod^⋆l.stars).tensor l.right) := rfl

@[simp]
lemma boundary_bottom (l : Layer C) :
    l.boundary .Bottom = l.left.tensor ((l.dom^⋆l.stars).tensor l.right) := rfl

inductive Hom : Layer C → Layer C → Type max (u + 1) v where
  | comp : Hom l₁ l₂ → Hom l₂ l₃ → Hom l₁ l₃
  | freeLeft (l : (L₁.as ⟶T L₂.as)) : Hom ⟨L₁, X, Y, s, x, R⟩ ⟨L₂, X, Y, s, x, R⟩ -- σ
  | freeRight (r : (R₁.as ⟶T R₂.as)) : Hom ⟨L, X, Y, s, x, R₁⟩ ⟨L, X, Y, s, x, R₂⟩ -- σ
  ...
  | box_strand_hom (L X Y s R A) {x : X.as ⟶ Y.as} :
      Hom ⟨L, X, Y, s, x, A.tensor R⟩ ⟨L.tensor A, X, Y, s, x, R⟩ -- σ underline
  | box_strand_inv (L X Y s R A) {x : X.as ⟶ Y.as} :
      Hom ⟨L.tensor A, X, Y, s, x, R⟩ ⟨L, X, Y, s, x, A.tensor R⟩
  | strand_box_hom (L X Y s R A) {x : X.as ⟶ Y.as} :
      Hom ⟨L.tensor A, X, Y, s, x, R⟩ ⟨L, X, Y, s, x, A.tensor R⟩
  | strand_box_inv (L X Y s R A) {x : X.as ⟶ Y.as} :
...
@[simp]
def φ {l₁ l₂ : Layer C} (b : TopBottom) : (l₁ ⟶L l₂) → ((l₁.boundary b).as ⟶T (l₂.boundary b).as)
  ...
  | box_strand_hom L X Y s R A => match b with
      | .Top => by simp ; exact (𝟙 _ ⊗⋆≫
          L.as ◁ (σ_ (Y^⋆s).as A.as).hom ▷ R.as ⊗⋆≫ 𝟙 _)
      | .Bottom => by simp ; exact (𝟙 _ ⊗⋆≫
          L.as ◁ (σ_ (X^⋆s).as A.as).hom ▷ R.as ⊗⋆≫ 𝟙 _)
  | box_strand_inv L X Y s R A => match b with
      | .Top => by simp ; exact (𝟙 _ ⊗⋆≫
          L.as ◁ (σ_ (Y^⋆s).as A.as).inv ▷ R.as ⊗⋆≫ 𝟙 _)
      | .Bottom => by simp ; exact (𝟙 _ ⊗⋆≫
          L.as ◁ (σ_ (X^⋆s).as A.as).inv ▷ R.as ⊗⋆≫ 𝟙 _)
  | strand_box_hom L X Y s R A => match b with
      | .Top => by simp ; exact (𝟙 _ ⊗⋆≫
          L.as ◁ (σ_ A.as (Y^⋆s).as).hom ▷ R.as ⊗⋆≫ 𝟙 _)
      | .Bottom => by simp ; exact (𝟙 _ ⊗⋆≫
          L.as ◁ (σ_ A.as (X^⋆s).as).hom ▷ R.as ⊗⋆≫ 𝟙 _)
  | strand_box_inv L X Y s R A => match b with
      | .Top => by simp ; exact (𝟙 _ ⊗⋆≫
          L.as ◁ (σ_ A.as (Y^⋆s).as).inv ▷ R.as ⊗⋆≫ 𝟙 _)
      | .Bottom => by simp ; exact (𝟙 _ ⊗⋆≫
          L.as ◁ (σ_ A.as (X^⋆s).as).inv ▷ R.as ⊗⋆≫ 𝟙 _)
