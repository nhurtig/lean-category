...
@[simp]
instance : MulOne (FQ C) where
  one := unit
  mul := tensor

@[simp]
instance : Star (FQ C) where
  star := star

inductive Hom : FQ C → FQ C → Type max u v
  | of (q : X ⟶ Y) : Hom X Y
  | id (X) : Hom X X
  | comp {X Y Z} (f : Hom X Y) (g : Hom Y Z) : Hom X Z
  | whiskerLeft (X : FQ C) {Y₁ Y₂} (f : Hom Y₁ Y₂) : Hom (X.tensor Y₁) (X.tensor Y₂)
  | whiskerRight {X₁ X₂} (f : Hom X₁ X₂) (Y : FQ C) : Hom (X₁.tensor Y) (X₂.tensor Y)
  | tensor {W X Y Z} (f : Hom W Y) (g : Hom X Z) : Hom (W.tensor X) (Y.tensor Z)
  | α_hom (X Y Z : FQ C) : Hom ((X * Y) * Z) (X * (Y * Z))
  | α_inv (X Y Z : FQ C) : Hom (X * (Y * Z)) ((X * Y) * Z)
  | l_hom (X) : Hom (1 * X) X
  | l_inv (X) : Hom X (1 * X)
  | ρ_hom (X : FQ C) : Hom (X * 1) X
  | ρ_inv (X : FQ C) : Hom X (X * 1)
  | star {X Y} (f : Hom X Y) : Hom X.star Y.star
  | χ_hom (X Y : FQ C) : Hom (X.star * Y.star) (Y * X).star
  | χ_inv (X Y : FQ C) : Hom (Y * X).star (X.star * Y.star)
  | ε_hom (X : FQ C) : Hom X.star.star X
  | ε_inv (X : FQ C) : Hom X X.star.star
  | twist_hom (X : FQ C) : Hom X.star X
  | twist_inv (X : FQ C) : Hom X X.star

infixr:10 " ⟶ᵐ " => Hom
-- TODO notation for 𝟙ᵐ, ≫ᵐ

inductive HomEquiv : ∀ {X Y : FQ C}, (X ⟶ᵐ Y) → (X ⟶ᵐ Y) → Prop
  ...
  | a {R : FQ C} : HomEquiv (Hom.ε_hom R).star (Hom.ε_hom R.star)
