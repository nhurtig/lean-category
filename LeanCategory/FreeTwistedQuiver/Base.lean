...
with additional free morphisms from a quiver.
These have the same shape as an object in the free involutive category
and free twisted category; again we define a new structure to avoid
...
@[inherit_doc FreeTwistedCategoryQuiver]
...
/--
Custom induction principle for `TQ C` that follows the structure of `I C`.
-/
...
/--
Map a function `C → D` over the object structure.
-/
...
/--
A premorphism in the free twisted category with a quiver (before quotienting
by the appropriate relations to get the true morphisms). The
only difference between this and the premorphisms in the free twisted category
are the `of` quiver morphisms.
-/
