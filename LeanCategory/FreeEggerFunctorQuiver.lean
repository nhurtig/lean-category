...
def projectMapAux : ∀ {X Y : F C}, (X ⟶ₘ Y) → (X.projectObj m ⟶ Y.projectObj m)
  | _, _, Hom.of f => M f
  | _, _, Hom.id _ => 𝟙 _
...
@[simp]
def projectMap (X Y : F C) : (categoryFreeTwistedCategoryQuiver.Hom X Y) →
    (X.projectObj m ⟶ Y.projectObj m) :=
  _root_.Quotient.lift (projectMapAux m M) <| by
    ...
    | whiskerLeft _ _ _ _ hf => dsimp only [projectMapAux, FreeTwistedCategory.projectObj]; rw [hf]
    | whiskerRight _ _ _ _ hf => dsimp only [projectMapAux, FreeTwistedCategory.projectObj]; rw [hf]
    | tensor _ _ hfg hfg' => dsimp only [projectMapAux]; rw [hfg, hfg']
    | tensorHom_def f g =>
        dsimp only [projectMapAux, FreeTwistedCategory.projectObj]
...
def project : F C ⥤ D where
  obj := FreeTwistedCategory.projectObj m
  map {X Y} f := projectMap m M X Y f
...
def projectFree : F C ⥤ F D := project (.of <| m ·) (⟦Hom.of <| M ·⟧)
